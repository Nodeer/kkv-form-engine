...
angular.module('nettineuvoja')
  ...
  .service('mainService', function($templateRequest, $compile, $window, $timeout, $rootScope, $log, storageService, DEBUG, FROM_EMAIL, apiService, KutiService) {
    ...
    this.notifyChangeSlide = notifyChangeSlide;
    ...
    /**
     * Notifies the Kuti API that the slide has been changed.
     * @param {string} from
     * @param {string} to
     * @param {object} session
     */
    function notifyChangeSlide(from, to, session) {
      //$log.debug('Notify slide transition ' + from + ' -> ' + to);
      //KutiService.changeSlide(from, to, session);
    }
